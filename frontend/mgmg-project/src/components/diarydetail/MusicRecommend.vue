...
export default {
  ...
  async beforeDestroy() {
    ...
    if (this.selectMusic == "good") {
      console.log("good 선택");
      await musicInterest(this.recommendNumber);
      // axios.post(process.env.VUE_APP_API_URL + `/api/diary/interestmusic/${this.recommendNumber}`, {
      //   headers: {
      ...
      console.log("bad 선택");
      await musicBad(this.recommendNumber);
    }
  },
};
</script>
